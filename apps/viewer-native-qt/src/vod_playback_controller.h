...
class VodPlaybackController : public QObject {
  Q_OBJECT
...
public:
  explicit VodPlaybackController(ApiClient *apiClient, QObject *parent = nullptr);
  ~VodPlaybackController() override;
...
private:
  ...
  struct PlayerSlotContext {
    VodPlaybackController *controller = nullptr;
    int slotIndex = 0;
  };
